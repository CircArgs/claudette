...
class ClaudeCLIClient:
    ...
    def __init__(
        ...
        if llm_config:
            ...
            self._cmd_summarizer = llm_config.cmd_summarizer
    ...
    def _run_one_shot(self, prompt: str, cmd_override: str = "") -> str:
        """Run CLI in one-shot mode and return the output."""
        template = cmd_override or self._cmd_one_shot
        cmd = _build_cmd(template, prompt)
    ...
    def summarize(self, thread: str) -> LLMResponse:
        ...
        text = self._run_one_shot(thread, cmd_override=self._cmd_summarizer)

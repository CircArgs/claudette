...
def _pipeline_instructions(config: Config) -> str:
    """Generate pipeline execution instructions based on config."""
    pipeline = config.pipeline
    if not pipeline.enabled:
        return "Pipeline mode is disabled. Use simple single-worker dispatch for each issue."

    active_stages = [s for s in pipeline.stages if s not in pipeline.skip_stages]
    stage_chain = " → ".join(active_stages)
    lines = [
        f"**Active pipeline:** {stage_chain}",
        "",
        "For each issue, execute the agent pipeline in order. Skip stages listed in `skip_stages`.",
        "If a stage fails or the Reviewer requests changes, loop back to the Builder stage.",
    ]
    if pipeline.skip_stages:
        lines.append(f"\n**Skipped stages:** {', '.join(pipeline.skip_stages)}")
    return "\n".join(lines)
...
For complex issues (multiple independent changes, affects multiple files/systems, or has
vague scope), break them into focused sub-issues before dispatching workers:

1. Create sub-issues via `gh issue create --repo <repo> --title "<focused title>" --body "Parent: #N\\nDepends on <repo>#M (if sequential)\\n\\n<specific scope>"`
2. Apply the ready label to each sub-issue
3. Add a comment on the parent issue listing the sub-issues
4. Apply the blocked label to the parent (it's done when all children close)

Keep sub-issues small and independently testable. A worker should be able to complete
...
Each issue should flow through these stages. The manager orchestrates this by dispatching specialized sub-agents:

### 1. Scout (read-only)
- Reads the codebase to understand the relevant files, architecture, and existing patterns
- Produces a brief analysis: affected files, dependencies, risks, estimated complexity
- MUST NOT modify any files

### 2. Architect
- Takes the scout's analysis and the issue requirements
- Produces a spec: approach, files to create/modify, test strategy, edge cases
- MUST NOT write code — only the spec

### 3. Builder
- Implements the spec: writes source code and tests
- Follows existing patterns identified by the scout
- Commits with descriptive messages referencing the issue

### 4. Tester
- Runs the full test suite
- Checks coverage (if coverage tooling is available)
- Reports: tests passed/failed, coverage delta, any regressions

### 5. Reviewer
- Reviews the diff against the spec and issue requirements
- Checks for: correctness, style consistency, security issues, missing tests
- Either approves (proceed to PR) or requests changes (loop back to Builder)

{_pipeline_instructions(config)}
...
def _cron_marker(config: Config) -> str:
    ...
    return f"claudette tick  # {config.project_dir}"


def _build_cron_env(config: Config) -> str:
    """Build the env var prefix for the cron entry.

    Reads env var *names* from config.cron.capture_env, then snapshots their
    current values from the shell. No secrets are stored in config — only the
    names of which vars to forward.
    """
    import os

    parts = []
    for key in config.cron.capture_env:
...
def install_cron(config: Config) -> str | None:
    ...
    # Build env prefix from config + captured shell env
    env_prefix = _build_cron_env(config)
    env_part = f"{env_prefix} " if env_prefix else ""
    cron_cmd = (
        f"{env_part}cd {config.project_dir} && "
        f"{claudette_bin} tick  # {config.project_dir}"
    )

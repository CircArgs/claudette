...
def cmd_issue_create(
    ...
    """Create a new issue on GitHub. Interactive when args are missing."""
    import subprocess
    ...
    if repo:
        ...
        console.print("[bold]Which repo?[/bold]")
    ...
    if body is None:
        body = click.prompt(
            "Description (optional, or 'e' to open editor)",
            default="",
            show_default=False,
        )
        if body.strip().lower() == "e":
            body = click.edit("") or ""

    # ── Dependencies ──
    if depends_on is None:
        dep = click.prompt(
            "Depends on (e.g. #38, owner/repo#12, blank for none)",
            default="",
            show_default=False,
        )
        if dep.strip():
    ...
    if depends_on:
        dep_ref = depends_on.strip()
        # Normalize: bare number → #N
        if dep_ref.isdigit():
            dep_ref = f"#{dep_ref}"
        # If no repo prefix and it's just #N, add the repo
        if dep_ref.startswith("#"):
            dep_ref = f"{repo_name}{dep_ref}"
        body = f"Depends on {dep_ref}\n\n{body}" if body else f"Depends on {dep_ref}"

    # ── Ready label ──
    if ready is None:
        ready = click.confirm("Mark as ready for claudette?", default=True)
    ...
    if body:
        ...
        if len(preview) > 5:
            console.print(f"  [dim]...({len(preview) - 5} more lines)[/dim]")
    if labels:
...
def cmd_discover(
    ...
    """Run all discovery functions, print results, and optionally file issues."""
    import subprocess
    ...
    if repo:
        repo_paths = [(repo, repo)]
    else:
        repo_paths = [(r.name, r.path) for r in config.repositories if r.path]

    if not repo_paths:
        console.print("[dim]No repositories configured with local paths.[/dim]")
        return

    extensions = config.discovery.file_extensions
    threshold = config.discovery.min_coverage_threshold

    # TODOs & FIXMEs — group by file for issue creation
    todos_by_repo: dict[str, list[dict]] = {}
    for name, path in repo_paths:
        todos = discover_todo_comments(path, extensions=extensions)
        if todos:
            todos_by_repo[name] = todos

    if todos_by_repo:
        console.print("[bold]TODOs & FIXMEs[/bold]")
        for _name, todos in todos_by_repo.items():
            for t in todos:
                console.print(
                    f"  {t['file']}:{t['line']}  "
                    f"[yellow]{t['type']}[/yellow]: {t['text'][:80]}"
                )
        console.print()

    # Coverage gaps
    coverage_by_repo: dict[str, list[dict]] = {}
    for name, path in repo_paths:
        gaps = discover_coverage_gaps(path, min_threshold=threshold)
        if gaps:
            coverage_by_repo[name] = gaps

    if coverage_by_repo:
        console.print(f"[bold]Coverage Gaps (< {threshold}%)[/bold]")
        for _name, gaps in coverage_by_repo.items():
            for g in gaps:
                console.print(
                    f"  {g['file']}  {g['coverage']}%"
                    f" ({g['missing_lines']} lines missing)"
                )
        console.print()
    ...
    for name, path in repo_paths:
        deps = discover_stale_dependencies(path)
        if deps:
            deps_by_repo[name] = deps

    if deps_by_repo:
        console.print("[bold]Dependencies[/bold]")
        for name, dep_list in deps_by_repo.items():
            for d in dep_list:
                console.print(
                    f"  {name}: {d['file']}"
                    f" ({len(d['dependencies'])} packages)"
                )
        console.print()

    if not todos_by_repo and not coverage_by_repo and not deps_by_repo:
        console.print("[dim]No work items discovered.[/dim]")
        return

    if not create and not dry_run:
        return

    # --- File GitHub issues from discovered work ---
    ready_label = _primary_label(config.github.labels.ready_for_dev)
    issues_to_create: list[tuple[str, str, str]] = []  # (repo, title, body)

    # Group TODOs by file into one issue per file
    for repo_name, todos in todos_by_repo.items():
        ...
        for filepath, items in by_file.items():
            title = f"Resolve {len(items)} TODO/FIXME in {filepath}"
            lines = [f"Found {len(items)} code annotations in `{filepath}`:\n"]
            for t in items:
                lines.append(
                    f"- **{t['type']}** (line {t['line']}): {t['text']}"
                )
            lines.append("\n*Auto-discovered by `claudette discover`*")
            issues_to_create.append((repo_name, title, "\n".join(lines)))

    # One issue per low-coverage file
    for repo_name, gaps in coverage_by_repo.items():
        for g in gaps:
            title = f"Improve test coverage for {g['file']}"
            body = (
                f"Coverage is at **{g['coverage']}%** "
                f"({g['missing_lines']} lines missing).\n\n"
                f"Target: ≥{threshold}% coverage.\n\n"
                "*Auto-discovered by `claudette discover`*"
            )
            issues_to_create.append((repo_name, title, body))

    if not issues_to_create:
        return

    console.print(f"[bold]{len(issues_to_create)} issues to create:[/bold]")
    for repo_name, title, _body in issues_to_create:
        console.print(f"  {repo_name}: {title}")

    if dry_run:
        console.print("\n[dim]Dry run — no issues created.[/dim]")
        return

    # Create issues via gh CLI
    created = 0
    for repo_name, title, body in issues_to_create:
        cmd = ["gh", "issue", "create", "--repo", repo_name, "--title", title, "--body", body]
        if ready_label:
            cmd.extend(["--label", ready_label])
        try:
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
            if result.returncode == 0:
                url = result.stdout.strip()
                ...
                console.print(
                    f"  [red]Failed:[/red] {repo_name}: {title}\n"
                    f"    {result.stderr.strip()[:100]}"
                )
        except (FileNotFoundError, subprocess.TimeoutExpired) as e:
            console.print(f"  [red]Error:[/red] {e}")

    console.print(f"\n{created}/{len(issues_to_create)} issues created.")


# ── Metrics ──────────────────────────────────────────────────────────────


def cmd_metrics(config: Config, days: int = 7) -> None:
    """Show metrics summary and daily breakdown."""
    from claudette.core.metrics import MetricsStore

    metrics = MetricsStore(config.state_dir)
    s = metrics.summary()
    ...
    rows = [
        ("Total ticks", str(s["total_ticks"])),
        ("Total sessions", str(s["total_sessions"])),
        ("Uptime", s["uptime"]),
        ("", ""),
        ("PRs opened", str(s["total_prs_opened"])),
        ("PRs merged", str(s["total_prs_merged"])),
        ("Approval rate", f"{s['approval_rate']}%"),
        ("PRs today", str(s["prs_today"])),
        ("", ""),
        ("Issues completed", str(s["total_issues_completed"])),
        ("Issues escalated", str(s["total_escalated"])),
        ("Stale requeued", str(s["total_stale_requeued"])),
        ("", ""),
        ("Errors (total)", str(s["total_errors"])),
        ("Errors today", str(s["errors_today"])),
    ]
    ...
    for label, value in rows:
        if not label:
            console.print()
            continue
        console.print(f"  {label:<22s} {value}")

    # Per-repo PR counts
    if s["prs_opened_by_repo"]:
        console.print()
        console.print("[bold]PRs by repo[/bold]")
        for repo, count in sorted(s["prs_opened_by_repo"].items()):
            merged = s["prs_merged_by_repo"].get(repo, 0)
            console.print(f"  {repo:<35s} opened: {count}  merged: {merged}")

    # Daily breakdown
    daily = metrics.daily_stats(days=days)
    console.print()
    console.print(f"[bold]Daily breakdown (last {days} days)[/bold]")
    console.print()

    # Header
    header = (
        f"  {'Date':<12s} {'Ticks':>6s} {'Sess':>6s} {'PRopen':>7s} "
        f"{'PRmrg':>6s} {'Issues':>7s} {'Errors':>7s} {'Escal':>6s}"
    )
    ...
    for day in daily:
        line = (
            f"  {day['date']:<12s} {day['ticks']:>6d} {day['sessions']:>6d} "
            f"{day['prs_opened']:>7d} {day['prs_merged']:>6d} "
            f"{day['issues_completed']:>7d} {day['errors']:>7d} {day['escalated']:>6d}"
        )
        console.print(line)
